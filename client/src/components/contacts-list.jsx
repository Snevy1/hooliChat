...
const ContactList = ({contacts, isChannel = false}) => {
    ...
    const {selectedChatData, setSelectChatData, setSelectedChatType, selectedChatType, setSelectedChatMessage } = useAppStore();
                ...
                <div key={contact._id} className={`pl-10 py-2 transition-all duration-300 cursor-pointer ${selectedChatData &&selectedChatData._id === contact._id ? "bg-[#8417ff] hover:bg-[#8417ff]":"hover:bg-[#f1f1f111]"}`} onClick={()=>handleClick(contact)}>
                    <div className="flex gap-5 items-center justify-start text-neutral-300">
...

                        {
                            isChannel ? <span>
                                {contact.name}
                            </span>: <span>{ contact.firstName ? `${contact.firstName} ${contact.lastName}`: contact.email }</span>
                        }
                    </div>
